...
const monnifyWebhook = async (req, res, next) => {
    ...
    try {
        ...
        const { paymentStatus, paymentReference, // the reference you passed to MonnifySDK.initialize()
        transactionReference, amountPaid, } = eventData;
        console.info(`[Monnify Webhook] Event received — status: ${paymentStatus}, ref: ${paymentReference}`);
        /**
        ...
        if (paymentStatus !== SUCCESSFUL_PAYMENT_STATUS) {
            console.info(`[Monnify Webhook] Skipping non-PAID event: ${paymentStatus}`);
            return;
        }
        if (!paymentReference) {
            console.error("[Monnify Webhook] No paymentReference in eventData", eventData);
            return;
        }
        /**
         ...
         */
        await Payment_1.PaymentService.verifyPayment(paymentReference);
        console.info(`[Monnify Webhook] Order verified successfully — ref: ${paymentReference}`);
    }
    catch (error) {
        /**
         ...
         */
        console.error("[Monnify Webhook] Processing error:", error?.message ?? error);
    }
};
